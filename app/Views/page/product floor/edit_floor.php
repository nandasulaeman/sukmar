...
<div class="card">
    <div class="card-header">
        <h4 class="card-title">Edit Data Denah Produk</h4>
    </div>
    <div class="card-body">
        <form class="form" method="post" action="<?= base_url('products_floor/update') ?>"
            enctype="multipart/form-data">
            <input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
            <div class="row">
                <div class="form-group">
                    <input type="hidden" name="floor_id" value="<?= $floorproduct->id; ?>">
                    <label for="id_product">Nama Produk</label>
                    <select class="form-control" name="id_product" id="id_product">
                        <?php
                        foreach ($product as $rows): ?>
                            <option value="<?= $rows->id; ?>" <?= ($floorproduct->id_product == $rows->id) ? 'selected' : ''; ?>>
                                <?= $rows->id; ?>-
                                <?= $rows->name_product; ?>
                            </option>
                        <?php endforeach; ?>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="id_floor" id="id_floor">
                        <?php foreach ($floorplan as $rows): ?>
                            <option value="<?= $rows->id; ?>" <?= ($floorproduct->id_floor_plan == $rows->id) ? 'selected' : ''; ?> data-src="<?= base_url('img/' . $rows->picture_floor); ?>">
                                <?= $rows->name_floor; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
